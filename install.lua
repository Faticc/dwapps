-- Установка и обновление программ DwOS из репозитория, одной командой:
--
--   wget -f https://raw.githubusercontent.com/Faticc/dwapps/main/install.lua /tmp/a.lua && /tmp/a.lua
--
-- Дальше обновляться - просто "dwapps-update": установщик кладёт себя в
-- каталог программ и помнит, откуда ставил.
--
--   --repo=владелец/репо   откуда качать (по умолчанию Faticc/dwapps)
--   --branch=ветка         ветка (main)
--   --dir=подкаталог       если файлы лежат не в корне репозитория,
--                          а, скажем, в dwapps/ - укажи --dir=dwapps
--   --to=/home/dwapps      куда ставить
...
-- хэш сошлись, - оборванная загрузка работающий банкомат не портит.
-- Качается не по имени ветки, а по хэшу коммита (ветка превращается в
-- него запросом к api.github.com): raw.githubusercontent держит ветку в
-- кэше до пяти минут и сразу после публикации отдал бы старый манифест.
-- Так же устроен update в самой DwOS. Файлы и ярлыки, которые ставил он
-- сам, а в манифесте их больше нет, удаляются; чужое в каталоге не
-- трогается. Что и с каким хэшем стоит, записано в <каталог>/.installed.
--
-- Библиотеки лежат не в /lib, а в подкаталоге программ (lib/ рядом с
-- ними): ярлык в /bin сам дописывает его в package.path, поэтому вся
-- установка - это одна папка и несколько ярлыков, и снести её можно, не
-- разбираясь, чей mcsrv.lua лежит в системном /lib.
...
--- считается посимвольно. Поэтому время от времени отдаём тик.
...
--- Откуда запущен установщик: если из каталога программ (ярлык
--- dwapps-update), то и ставить туда же, и репозиторий брать тот же.
...
if not TO0 then
  local d = selfDir()
  TO0 = (d and fs.exists(d .. "/" .. STATE_NAME)) and d or "/home/dwapps"
end
local old = readState(TO0) or {}

local REPO   = opts.repo or old.repo or "Faticc/dwapps"
...
do
  local ok, h = pcall(internet.request,
    ...
    { ["user-agent"] = "dwapps", ["accept"] = "application/vnd.github.sha" })
  if ok and h then
    local body = {}
...
--- на необязательные файлы (opt в манифесте) он не жалуется.
local function open(path)
  local url = ("https://raw.githubusercontent.com/%s/%s/%s%s"):format(REPO, REF, SUB, path)
  local ok, h = pcall(internet.request, url, nil, { ["user-agent"] = "dwapps" })
...
--- Скачать прямо в файл, не собирая его в памяти, и заодно посчитать
--- хэш. Возвращает размер и хэш.
...
--- Два раза пробуем: raw.githubusercontent иногда рвёт ответ на середине.
...
print(("Программы: %s@%s%s%s"):format(REPO, BRANCH,
...
-- Чего программам не хватает, говорим до установки, а не когда игрок
-- запустит банкомат и увидит пустой экран. Ничего из этого не повод
-- отказаться ставить: машину достраивают уже после.
do
  local gpu = component.isAvailable("gpu") and component.gpu
  if not gpu then die("нужна видеокарта") end
  local w, h = gpu.maxResolution()
  if w < 88 or h < 25 then
    print(("  внимание: экран %dx%d - банкомату хватит 49x24, а admins"):format(w, h))
    print("  просит 88x25: нужны видеокарта и монитор 3 уровня")
  end
  if gpu.getDepth and gpu.getDepth() < 8 then
    print("  внимание: видеокарта не 3 уровня - цвета будут приблизительными")
  end
  if not gpu.allocateBuffer then
    print("  внимание: видеопамяти нет, кадр будет рисоваться прямо на экран")
  end
  -- в DwOS gfx может быть уже загружен, минуя диск
  local hasGfx = package.loaded.gfx ~= nil
  if not hasGfx and package.searchpath then
    hasGfx = package.searchpath("gfx", package.path) ~= nil
  end
  if not hasGfx then
    print("  внимание: библиотеки gfx нет - это программы для DwOS,")
    print("  в обычной OpenOS они не запустятся")
  end
  if not component.isAvailable("opencb") then
    print("  внимание: командного блока (opencb) нет - программы запустятся,")
    print("  но денег и заходов игроков не увидят: спрашивать не у кого")
  end
end
...
-- Ярлыки: чтобы запускать, набрав "bank" из любого каталога --------------------
...
-- Библиотеки лежат рядом с программами, а не в системном /lib, поэтому
-- каталог с ними ярлык дописывает в package.path сам.
local LIB
if manifest.lib then
  local sub = manifest.lib:gsub("^/+", ""):gsub("/+$", "")
  if sub ~= "" then LIB = TO .. "/" .. sub end
end
...
for _, b in ipairs(manifest.bin or {}) do
  ...
  if cur ~= body then
    ...
    if DRY then print("   (проба)") else
      -- /bin бывает и на дискете только для чтения: программы уже
      -- стоят, так что это не повод обрывать установку
      mkdir(path)
...
print("  bank          - банкомат: снять со счёта деньги предметом и внести обратно")
print("  admins        - мод-состав: кто в какой группе и когда заходил")
print("  cbtest        - показать, что сервер отвечает на команду")
print("  dwapps-update - обновить программы")
